document.addEventListener('DOMContentLoaded', function() {
    ...
    // تهيئة Firebase - استخدام Firebase compat للتوافق
    try {
        if (typeof firebase !== 'undefined') {
            ...
            console.log('تم تهيئة Firebase بنجاح والوصول إلى Firestore');
        } else {
            console.error('Firebase SDK غير متوفر. تأكد من تضمين مكتبات Firebase في HTML قبل ملف script.js.');
        }
    } catch (error) {
...
function updateGalleryColorSelection(activeColor) {
    ...
    colorOptions.forEach(option => {
        ...
        if (productGalleryImages[color] && productGalleryImages[color].length > 0) {
            ...
            // Hide if it has no images, even if it was the target active color
...
/**
 * التنقل بين صور المعرض (السابق/التالي)
 * @param {string} direction - 'prev' أو 'next'
 */
function navigateGallery(direction) {
    const imagesForCurrentColor = productGalleryImages[galleryCurrentColor];

    // Ensure the color exists and has more than one image for navigation
    if (!imagesForCurrentColor || imagesForCurrentColor.length <= 1) {
        // console.log('لا يمكن التنقل، صورة واحدة أو لا توجد صور لهذا اللون.');
        return;
    }
    ...
    if (direction === 'prev') {
        ...
        // console.log(`التنقل للسابق: الفهرس الجديد ${newIndex}`);
    } else { // next
        newIndex = (galleryCurrentImageIndex + 1) % totalImages;
         // console.log(`التنقل للتالي: الفهرس الجديد ${newIndex}`);
    }

    // Only update if the index actually changed (might not if totalImages is 0 or 1, though we check above)
...
/**
 * تحديث عداد الصور (مثل 1 / 5)
 * @param {number} current - رقم الصورة الحالية (يبدأ من 1)
 * @param {number} total - إجمالي عدد الصور للون المحدد
 */
function updateGalleryCounter(current, total) {
    ...
    if (total > 1) {
        ...
        counterElement.style.display = ''; // Show counter
    } else {
        counterElement.style.display = 'none'; // Hide counter if 1 or 0 images
    }
}
 ...
 * @param {string} color - كود اللون (black, white, silver, blue, beige)
...
function initEnhancedTestimonials() {
    ...
    let currentSlide = 0;
    let autoSlideTimer;
    const autoSlideInterval = 8000; // 8 ثوان
    
    // إظهار البطاقة الأولى
    showTestimonial(0);
    ...
    // إيقاف التمرير التلقائي عند التفاعل مع عنصر السلايدر
...
// دالة لتحويل الأرقام العربية إلى الأرقام الإنجليزية
function convertArabicToEnglishNumbers(str) {
    if (!str) return '';
    
    // جدول تحويل الأرقام العربية إلى إنجليزية
    ...
    // استبدال كل رقم عربي بما يقابله في الإنجليزية
    return str.replace(/[٠١٢٣٤٥٦٧٨٩]/g, match => arabicToEnglish[match]);
}

// إضافة مستمع حدث لتحويل الأرقام العربية في حقل رقم الهاتف
document.addEventListener('DOMContentLoaded', function() {
    const mobileField = document.getElementById('mobileNumber');
    if (mobileField) {
        mobileField.addEventListener('input', function() {
            // تحويل الأرقام العربية إلى إنجليزية مباشرة أثناء الكتابة
            const cursorPosition = this.selectionStart;
            const originalLength = this.value.length;
            this.value = convertArabicToEnglishNumbers(this.value);
            
            // الحفاظ على موضع المؤشر بعد التحويل
...
// إضافة تأثير الهيدر عند التمرير
...
/**
 * إدارة النافذة المنبثقة لمخطط المقاسات
 */
function openSizeChart() {
    ...
    if (sizeChartSection) {
        // التمرير إلى قسم المقاسات بطريقة سلسة
        sizeChartSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
        
        // في حالة وجود تبويبات في قسم المقاسات، تنشيط التبويب الأول (تبويب حساب المقاس)
        ...
        // إضافة تأثير وميض لجذب الانتباه
...
function initSizeChartModal() {
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', function() {
            // إزالة الفئة النشطة من جميع علامات التبويب والمحتويات
            document.querySelectorAll('.size-chart-tab').forEach(t => t.classList.remove('active'));
            document.querySelectorAll('.tab-pane').forEach(p => p.classList.remove('active'));
    ...
    // ربط جميع أزرار فتح جدول المقاسات بالدالة
...
function initSizeCalculator() {
    ...
    if (inlineTabs && inlineTabs.length > 0) {
        inlineTabs.forEach(tab => {
            tab.addEventListener('click', function() {
                ...
                // تنشيط المحتوى المرتبط بالتبويب
    ...
    // تبديل وحدات القياس في النافذة المنبثقة
    initUnitToggle('.unit-btn', changeUnits);
    
    // تبديل وحدات القياس للجدول
    initUnitToggle('.unit-toggle', toggleChartUnits);
    
    // تبديل وحدات القياس في القسم المضمن - جدول المقاسات
    ...
    if (buttons && buttons.length > 0) {
        buttons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                // تغيير الوحدات بناءً على الاختيار
...
// حساب المقاس الموصى به في القسم المضمن
...
function calculateSize(height, weight) {
    // منطق تحديد المقاس بناءً على الطول والوزن معاً
    let size = '';
...
// تحديث موضع وقيمة فقاعة شريط التمرير
...
// تبديل وحدات القياس (متري / إمبريالي)
function changeUnits(unit) {
    ...
    if (unit === 'metric') {
        // تحويل إلى وحدات مترية
        if (heightSlider) {
            heightSlider.min = "150";
            heightSlider.max = "210";
            
            // تحويل القيمة الحالية من إنش إلى سم إذا كانت بالإنش
            if (heightSlider.getAttribute('data-unit') === 'imperial') {
        ...
        if (weightSlider) {
            ...
            // تحويل القيمة الحالية من رطل إلى كجم إذا كانت بالرطل
            if (weightSlider.getAttribute('data-unit') === 'imperial') {
        ...
        if (heightSlider) {
            heightSlider.min = "59"; // 4'11"
            heightSlider.max = "83"; // 6'11"
            
            // تحويل القيمة الحالية من سم إلى إنش إذا كانت بالسم
        ...
        if (weightSlider) {
            weightSlider.min = "88"; // 88 رطل
            weightSlider.max = "352"; // 352 رطل
            
            // تحويل القيمة الحالية من كجم إلى رطل إذا كانت بالكجم
    ...
    // إعادة حساب المقاس الموصى به
    updateRecommendedSize();
}

// تبديل وحدات جدول المقاسات
...
// تحديث المقاس الموصى به عند التحميل
function updateRecommendedSize() {
    ...
    // تحسين خوارزمية حساب المقاسات بناءً على الطول والوزن
    if (unit === 'metric') {
        // المقاسات بالنظام المتري (سم/كجم)
        if (height <= 160) {
            if (weight <= 60) {
                size = 'S';
            } else if (weight <= 75) {
        ...
        // المقاسات بالنظام الإمبراطوري (إنش/رطل)
        if (height <= 63) {
            if (weight <= 132) {
                size = 'S';
            } else if (weight <= 165) {
    ...
    // تحديث عنصر المقاس الموصى به
    recommendedSize.textContent = size;
    
    // تأثير بصري لتغيير المقاس
...
docReady(function() {
    ...
    initSizeCalculator();
    initGalleryZoom();
    initSizeChartSection(); // تهيئة قسم جدول المقاسات
    initInlineSizeChart(); // استدعاء الدالة الجديدة
    // ... existing code ...
});

// ... existing code ...

// Inicializar todos los elementos del formulario de pedido
function initOrderForm() {
    ...
    // Inicializar la selección de talla
    initSizeSelection();
    
    // Inicializar los controles de cantidad
    initQuantityControls();
    
    // Inicializar botones para añadir/eliminar productos
    initProductControls();
    
    // Inicializar el cálculo del precio total
    updateTotalPrice();
...
function initColorSelection() {
    ...
    colorOptions.forEach(option => {
        option.addEventListener('click', function() {
            ...
            // Eliminar la clase active de todas las opciones de color
            ...
            // Actualizar imagen de vista previa y texto del color seleccionado
...
// Inicializar selección de talla
function initSizeSelection() {
    ...
    sizeButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Eliminar la clase active de todos los botones de talla
            ...
            // Actualizar el texto de la talla seleccionada
...
// Actualizar vista previa del producto al cambiar el color
function updateProductPreview(productItem, color) {
    ...
    if (previewImg) {
        ...
        let imagePath = '';
        // Use a representative image for each color
        switch (color) {
            case 'black':
                imagePath = 'Public/images/black/black-tshirt.jpg'; // Use black t-shirt image
                break;
            // REMOVED 'white' case
            case 'silver':
                imagePath = 'Public/images/silver/silver-tshirt.jpg'; // Use silver t-shirt image
                break;
            case 'beige': // ADDED beige case
                imagePath = 'Public/images/beige/beige-tshirt.jpg'; // Use beige t-shirt image
                break;
            default:
                imagePath = 'Public/images/black/black-tshirt.jpg'; // Default to black
        }

        previewImg.src = imagePath;
        // Update alt text as well
        previewImg.alt = `طقم HELDEN ${getColorName(color)}`; // Use getColorName for consistency
    }

    // Actualizar texto del color seleccionado
    const previewColor = productItem.querySelector('.product-preview-color');
    if (previewColor) {
        previewColor.textContent = `اللون: ${getColorName(color)}`; // Use getColorName for consistency
    }
}
...
function initProductControls() {
    ...
    if (addProductBtn && selectedProductsContainer) {
        // Contador para los IDs de productos
        let productCounter = 1;
        
        // Al cargar la página, asegúrese de que al menos hay un producto
        const existingProducts = selectedProductsContainer.querySelectorAll('.product-item');
        productCounter = existingProducts.length;
        
        // Si ya hay productos, inicialice el contador correctamente
        if (productCounter > 0) {
            productCounter = Math.max(...Array.from(existingProducts).map(item => 
                parseInt(item.getAttribute('data-product-id')) || 0
            ));
        }
        
        // Configurar el botón para añadir un producto
        ...
        // Configurar botones para eliminar productos existentes
        setupRemoveButtons();
        
        // Configurar botones para añadir productos con descuento
...
function addNewProduct(productId) {
    ...
    // Contenido HTML del nuevo producto
    newProduct.innerHTML = `
        ...
        <div class="product-item-summary">
            <div class="product-preview">
                <img src="Public/images/order-color/black-tshirt.jpg" alt="تي شيرت أسود" class="product-preview-img">
            ...
            <div class="product-price">
                <span class="item-price">699 ج.م</span>
            </div>
        </div>
    `;
    ...
    // Inicializar los eventos para el nuevo producto
    initColorSelection();
    initSizeSelection();
    initQuantityControls();
    setupRemoveButtons();
...
function setupRemoveButtons() {
    ...
    removeButtons.forEach(button => {
        ...
        newButton.addEventListener('click', function() {
            ...
            // Comprobar si hay al menos un producto antes de eliminar
    ...
    if (addOutfitDiscountBtn) {
        addOutfitDiscountBtn.addEventListener('click', function() {
            ...
            if (allProducts.length < 2) {
                ...
                console.log('Añadido segundo producto con descuento del 10%');
            } else {
    ...
    if (addThirdOutfitBtn) {
        addThirdOutfitBtn.addEventListener('click', function() {
            ...
            if (allProducts.length < 3) {
                ...
                console.log('Añadido tercer producto con descuento del 15%');
            } else {
...
function updateTotalPrice() {
    ...
    if (totalPriceElement) {
        let basePrice = 749; // السعر الأساسي للقطعة الواحدة
        let totalPrice = 0;
        let discountedPrice = basePrice;
        let discountApplied = false;
        
        // حساب السعر حسب عدد المنتجات والخصومات
        if (totalItems === 1) {
            ...
            // خصم 15% للقطعة
            discountedPrice = Math.round(basePrice * 0.85);
        ...
        if (discountElement) {
            ...
            if (discountApplied) {
                ...
                discountElement.style.color = 'var(--success-color)'; // لون أخضر للإشارة للخصم
            } else {
                discountElement.textContent = `0 ج.م (0%)`;
                discountElement.style.color = ''; // إعادة اللون الافتراضي
            }
        }
        
        // إظهار/إخفاء وتفعيل/تعطيل طبقات الخصم
        ...
        if (originalTotalElement) {
            originalTotalElement.textContent = `${basePrice * totalItems} ج.م`;
        }
        
        // إظهار إشعار بسيط عند تطبيق الخصم
...
function submitOrder() {
    ...
    const orderData = {
        id: orderNumber, // تأكد من أن ID هو نفسه رقم الطلب
    ...
    try {
        ...
        db.collection('orders').doc(orderNumber).set(orderData)
            .then(() => {
                console.log('Pedido guardado correctamente con número:', orderNumber);
                ...
                // Dejar solo un producto en el formulario
